...
import com.taobao.demo.util.InterfaceProxyUtils;
import com.taobao.demo.util.JDBCUtils;
...
public class LogServletAccessModule implements Module, LoadCompleted {
     ...
     * <p>
     * 通过代理模式{@link InterfaceProxyUtils#puppet(Class, Object)} }调用
     ...
     * 记录到数据库
     */
    private void saveAccess(final HttpAccess ha,
    final long costMs,
    final Throwable cause) {
        HttpDO httpDO = new HttpDO();
        httpDO.setCause(cause.getMessage());
        httpDO.setFrom(ha.from);
        httpDO.setCost(costMs);
        httpDO.setStatus(ha.status);
